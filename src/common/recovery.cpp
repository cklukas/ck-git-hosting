...
// Streaming SHA-256 avoids a platform-specific checksum executable or library.
class Sha256 {
 public:
  ...
  std::string finish() {
    ...
    for (unsigned index = 0; index < 8; ++index) length[index] = static_cast<unsigned char>(bits >> ((7 - index) * 8));
    add(length, 8);
 ...
 private:
  void compress() {
    ...
    for (unsigned index = 0; index < 64; ++index) {
      const auto first = h + (std::rotr(e,6) ^ std::rotr(e,11) ^ std::rotr(e,25)) + ((e & f) ^ (~e & g)) + constants[index] + words[index];
...
std::string checksum(const fs::path& path) {
  ...
  while (true) {
    const auto count = read(fd, buffer.data(), buffer.size());
    if (count < 0 && errno == EINTR) continue;
    if (count < 0) { close(fd); throw std::runtime_error("could not checksum " + path.string()); }
    if (count == 0) break;
...
std::string manifest(const fs::path& root, BackupSummary* summary) {
  ...
  for (const auto& relative : treeEntries(root)) {
    ...
    if (fs::is_directory(path)) result += "D " + hexPath(relative.generic_string()) + "\n";
      ...
      result += "F " + hexPath(relative.generic_string()) + " " + std::to_string(bytes) + " " + checksum(path) + "\n";
      ++summary->files;
      summary->bytes += bytes;
    }
