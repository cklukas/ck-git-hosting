...
| `/usr/bin/ck-git-hostingd`, `ck-git-shell`, `ckgit-admin`, `ck-ci-runnerd`, `ck-pagesd`, `ck-git-hosting-deploy` | `root:root 0755` | server binaries |
...
| `/var/lib/ck-git-hosting/ci-cache` | `ckgit:ckgit 0700` | persistent per-project build caches (see [04-ci-cd.md](04-ci-cd.md)) |
...
| `/etc/systemd/system/ck-pages.service` | `root:root 0644` | hardened Pages server unit; enabled like the other two, but stays inactive until `pages_http_port` is also set |
